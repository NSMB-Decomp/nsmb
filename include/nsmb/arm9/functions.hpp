#pragma once

#include "../../../src/base_types.hpp"

class Base;
class Fader;
class PlatformMgr;
class PlayerActor;
class VSConnectScene;
class Vec2_32;
class Vec3_32;
struct FxRect;
struct FxRect;
struct Save;
struct MinigamesSave;
struct OptionSave;

extern "C" {
void InitMiniGame();
void func_0200419c(u32, u32, u32, u32, u32, u32, u32, u32, u32, u32, s32);
void func_02043fe0(Vec2_32 *, Vec2_32 *, s32, s32, s32);
bool func_02010a14(u8);
bool func_02010a3c();
bool func_02010a60(VSConnectScene *);
void func_0200d87c(void *, s32, s32, s32, s32, s32, s32, s32, s32, s32);
void func_02003e20(void *);
void func_02018060(void *);
void func_0200f608();
void func_0200f65c();
void func_0200f3d8();
void func_0200e944(u32, u32, u32);
void func_02017a80(void *);
void func_02017a90(void *);
void func_02016438(void *, const void *, u8);
s32 func_02046c78();
s32 func_0200f1b4(u8);
void func_02003f3c(void *);
void func_02003ed8(void *);
void func_02046b80();
s32 func_0200ec2c();
s32 func_02047694();
s32 func_02010b34(void *, void *);
void func_0200e7c4(u32);
s32 func_0200e7ac(u32);
void func_0200e7b8(u32);
void func_0200e748();
void func_0201e00c();
void func_020180c0(void *, u32);
void func_02011d24(u32);
void func_02012314(u32, u32);
void func_02011c34(u32);
void func_02008558(Fader *);
bool func_02046c5c();
void func_0200e874(u32, u32, bool);
void func_0201325c();
void func_02010a84();
void func_020125c4();
u16 func_02043398(u16 *);
void func_02006444(u32);
u32 func_020128c4(MinigamesSave *);
u32 func_020128fc();
bool func_02012ee0(s32, Save *);
u32 func_02012f64(s32, Save *);
u32 func_02013050(OptionSave *);
u32 func_02013090();
u32 func_020130b4();
void func_02018ac0();
void func_02008980();
u32 func_02009acc(u32);
void func_02006ca4();
void func_0201df50();
void func_02020628();
void func_02017e14(void *, const u32 *);
void func_0200dee0(u32, u32);
void func_02011d60(u32);
bool func_0200ee40(s32);
void func_0201dfe8();
void *func_020232e8();
void func_0200ee9c(void *, u32, u32, u32);
void func_02018b40();
int strncmp(const char *, const char *, u32);
}

extern "C" void func_02011b64();
void func_02045128();
void func_0204d908();
void func_02044ab0(Base *, void *);
void func_020201c8(i32, i32);
extern "C" void func_02022b64(u32, Vec3_32 *);
void func_02020150(u32, u32);
void func_02020128(u32, u32);
void func_020122d0(u32, u32, u32);
void func_02012290(u32, u32);
void func_0201224c(u32, u32, u32);
void func_020121e4(u32);
void func_0204e87c(u32, u32);
void func_02011e3c(u32);
void func_02011dc4(u32);
void func_02011d94(u32);
void func_0202048c(i8);
void func_02012d6c(u32, Save *);
void func_020131fc(u32, u32);
void func_02014820(void *);
void func_0201481c(void *);
void func_0201d794(PlatformMgr *);
extern "C" void func_0201d954(PlatformMgr *, u32);
extern "C" void func_0201eef8(u32, FxRect *);
extern "C" void func_02020300(u32, u32);
extern "C" void func_02020544(u32);
extern "C" PlayerActor *func_020205ec();
extern "C" void func_02022220(Vec3_32 *);
extern "C" void func_020221d8(Vec3_32 *);
extern "C" void func_020221b4(Vec3_32 *);
extern "C" void func_020221fc(Vec3_32 *);
extern "C" u32 func_0200ae9c(Vec3_32 *, FxRect *, s8);
extern "C" bool func_0200aed8(Vec3_32 *, FxRect *, u8);
extern "C" void *func_0201f184(u8, FxRect *);
void func_02011c64();
void sceneBaseInit();
u32 data_02088f18();
extern "C" u32 func_02006674(u32);
extern "C" u32 func_02023328(u16);
