...
i32 Base::doOrderProc()
{
	if (this->pending_destroy) {
		...
		this->state = Inactive;
		for (SceneNode *first_child = this->process_link.connect.firstChild; first_child != (void *)0x0; first_child = first_child->next) {
		...
		} else if (BOOL(this->state == Inactive) == FALSE) {
...
{
	if (!((this->_54 == 0 || func_0204d82c()) && this->process_link.connect.firstChild == (SceneNode *)0x0))
...
void Base::destroy()
{
	if ((this->pending_destroy == false) && (BOOL)(this->state == Inactive) == FALSE) {
...
bool Base::prepareResourcesSafe(u32 size, Heap *parent)
{
	...
	if (size != 0 && (user = FrameHeap::create(size, parent, 0x20), user != NULL)) {
		...
		heap->setCurrent();
		if (heap_ptr == NULL && (z = user->allocate(0x10, 0x10), z == NULL)) {
			result = 0;
		}
...
bool Base::hasChildPendingCreation()
{
	...
	while (!(cur == (SceneNode *)0x0 || cur == next)) {
		if ((u8)((*cur->object).state == PendingInit) != 0) {
			return true;
		}
